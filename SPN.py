...
def filter_main(folder, h, w, nocrop):
    ...
    size = len(allfiles)
    est_camera_ref = []
    imlist = [filename for filename in allfiles if filename[-4:] in [".jpg", ".JPG", '.png']]
    for f in imlist:
        ...
        print('\r|{}|{}% Time elapsed: {}, estimate: {}'.format(("█" * progressbar), frogress, elapse, est_time),
              end="", flush=True)
        j += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nocrop", help="python filter_cy Folder_Path height width (Default 256x256)", dest='nocrop',
                        action='store_true')
